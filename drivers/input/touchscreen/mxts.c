...
static void mxt_report_input_data(struct mxt_data *data)
{
	...
	if(data->mxt_enabled == 1){//0912
		...
		if((data->Exist_Stylus > 1)&&(data->Exist_Stylus != 100)&&(data->clear_cover_enable != 1)){//0912
			...
			mxt_write_object(data, MXT_PROCI_STYLUS_T47, 2, 1);
			mxt_write_object(data, MXT_TOUCH_MULTITOUCHSCREEN_T9, 7, 50);
		}
...
//	dev_info(&data->client->dev,  "[TSP] T57: tchnum = %d, total= %d, tch=%d, atch=%d\n", data->Report_touch_number,total_area,tch_area, atch_area);
    ...
    case MXT_GR_GDC_STATUS_FINISH:
		...
		if( data->Old_Report_touch_number <= data->Report_touch_number){//0913_2
/* //0913_3
			dev_info(&data->client->dev, "Old_RN= %d, RN= %d \n",data->Old_Report_touch_number,data->Report_touch_number);//0913_2
		 	if((data->Report_touch_number > 1) && (atch_area > 10) && (tch_area < 5) && (total_area > (tch_area*8))&&(total_area > 50)){//0913_1
				dev_info(&data->client->dev, "T57 Disable GR Because palm touch after calibration case \n");
				dev_info(&data->client->dev,  "tchnum = %d, total= %d, tch=%d, atch=%d\n",
					data->Report_touch_number,total_area,tch_area, atch_area);
				mxt_gdc_init_config(data);
...
*/
			if((data->Report_touch_number>1) && (atch_area < 3) && (tch_area < 1 ) && (total_area > 11)){//0913_1
